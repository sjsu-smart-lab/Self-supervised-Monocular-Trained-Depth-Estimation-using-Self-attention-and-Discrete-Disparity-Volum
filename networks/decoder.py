...
class SoftAttnDepth(nn.Module):
    def __init__(self, alpha=0.01, beta=1.0, dim=1):
...
class MSDepthDecoder(nn.Module):
    def __init__(self,
        ...
        # (48, 160) -> (96, 320)
        self.deconv4 = UpBlock(in_planes=32,
                               out_planes=32,
                               use_skip=True,
                               skip_planes=64,
                               is_output_scale=True,
        ...
        # (96, 320) -> (192, 640)
        self.deconv5 = UpBlock(in_planes=32,
                               out_planes=16,
                               use_skip=False,
                               skip_planes=64,
                               is_output_scale=True,
        ...
        self.depth_layer = SoftAttnDepth(alpha=alpha, beta=beta)
    ...
    def forward(self, features):
        ...
        # decoder
        x = features[-1]
        x, z = self.deconv1(x, None)

        x, z = self.deconv2(x, None)
        self.outputs[("disp", 3)] = self.depth_layer(z)

        x, z = self.deconv3(x, features[1])
        self.outputs[("disp", 2)] = self.depth_layer(z)

        x, z = self.deconv4(x, features[0])
        self.outputs[("disp", 1)] = self.depth_layer(z)

        x, z = self.deconv5(x, None)
